...
class TestCompoundMesh(unittest.TestCase):
    ...
    def test_1(self):
        ...
        grid2d = Grid(size=(100, 100), shape=(4, 4), 
                      eshape='Q4', frame=A)
        grid3d = Grid(size=(100, 100, 20), shape=(4, 4, 2), 
                      eshape='H8', frame=A)
        ...
        self.assertTrue(np.all(np.isclose(mesh['tri'].center(), 
                                          [50., 50., -50.])))
        self.assertTrue(np.all(np.isclose(mesh['grids', 'Q4'].center(), 
                                          [50., 50., 50.])))
        self.assertTrue(np.all(np.isclose(mesh['grids', 'H8'].center(), 
                                          [50., 50., 10.])))
