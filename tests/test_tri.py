...
class TestTri(unittest.TestCase):
    ...
    def test_area_T3(self):
        def test_area_T3(Lx, Ly, nx, ny):
            try:
                ...
                mesh = TriMesh(size=(Lx, Ly), shape=(nx, ny), 
                               frame=A, celltype=T3)
    ...
    def test_area_circular_disk_T3(self):
        def test_area_circular_disk_T3(min_radius, max_radius, 
                                       n_angles, n_radii):
            try:
                mesh = circular_disk(n_angles, n_radii, min_radius, 
                                     max_radius)
    ...
    def test_area_circular_disk_T6(self):
        def test_area_circular_disk_T6(min_radius, max_radius, 
                                       n_angles, n_radii):
            try:
                mesh = circular_disk(n_angles, n_radii, min_radius, 
                                     max_radius)
